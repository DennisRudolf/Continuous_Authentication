...
def save_auth_values(session_id, p, q, g, h, d, C, e, w, session_timer):
    ...
    with open(csv_file, mode='a', newline='') as file:
        ...
        if not file_exists:
            writer.writerow(['session_id', 'p', 'q', 'g', 'h', 'd', 'C', 'e', 'w', 'session_timer'])
        row = [session_id, p, q, g, h, d, C, e, w, session_timer]
        ...
        for row in reader:
            if row['session_id'] == session_id:
                ...
                return (
                    ...
                    row['d'], row['C'], int(row['e']), int(row['w']), row['session_timer']
                )
...
def generate_validate_challenge_e(challenge_e_non_checked, commitment_c, commitment_d, e):
    input_data = f"{commitment_c}{commitment_d}{e}".encode()
